...
//threading function
void lychrelThreads(Number &number, size_t &iter, Number &n){
        ...
        while (!n.is_palindrome() && ++iter < MaxIterations) {
            ...
            transform(n.rbegin(), n.rend(), sum.rbegin(), 
                [&](auto d) {
                    auto v = d + *rd++ + carry;
...
int main() {
    ...
    cerr << "Processing " << data.size() << " values ...\n";
    ...
    for (auto i = 0; i < data.size(); ++i) {
        ...
        //threads here//
        thread(lychrelThreads, number, iter, n);
        // The Lychrel loop - for any iteration, take the number, reverse
        //   its digits, and sum those values together.  If that sum
        //   is a palindrome, stop processing
        /*while (!n.is_palindrome() && ++iter < MaxIterations) {
